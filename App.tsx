...
const App: React.FC = () => {
  ...
  // AtlasAIV2 v2.6 Voice Memory Integration with full default merging
  ...
  const [brandProfile, setBrandProfile] = useState<BrandVoiceProfile>(() => {
    ...
    let base = saved ? { ...defaultProfile, ...JSON.parse(saved) } : defaultProfile;
    
    // Ensure advanced settings nested object is also merged
    if (saved) {
      const parsedSaved = JSON.parse(saved);
      if (parsedSaved.advanced) {
        base.advanced = { ...defaultProfile.advanced, ...parsedSaved.advanced };
      }
    }

    if (memory?.enabled !== false && memory?.profileOverrides) {
       return { ...base, ...memory.profileOverrides };
    }
    return base;
  });
  ...
  }, [messages, audioChunks, isThinking, isAppInitialized]);
            ...
            setMessages(prev => {
              ...
              if (lastMsg && lastMsg.role === role && Date.now() - lastMsg.timestamp < 3000) {
                ...
                updated[updated.length - 1] = { 
                  ...
                  telemetry: { ...lastMsg.telemetry, ...telemetry }
                };
                return updated;
              }
              return [...prev, { id: `${role}-${Date.now()}`, role, content: text.trimStart(), timestamp: Date.now(), telemetry }];
            });
          },
          (engine) => setActiveEngine(engine),
          (state) => setSessionStatus(state),
          () => {
            setSessionStatus('idle');
            setIsHandsFree(false);
            setIsSpeaking(false);
  ...
  const getStatusLabel = (status: SessionState) => {
    switch (status) {
      case 'listening': return "Listening...";
      case 'validating': return "Validating Audio...";
      case 'committing': return "Committing Turn...";
      case 'responding': return "Awaiting Sync...";
      case 'playing': return "Synthesizing...";
      case 'recovering': return "Recovering Neural Link...";
  ...
  return (
    <>
      {!isAppInitialized && <SplashScreen onComplete={() => setIsAppInitialized(true)} />}
      
      <div className={`flex flex-col h-screen w-full max-w-lg mx-auto bg-black relative overflow-hidden transition-all duration-1000 ${isAppInitialized ? 'opacity-100' : 'opacity-0 scale-95'}`}>
            ...
            <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} animate-in slide-in-from-bottom-2`}>
              ...
              <div className="flex items-center gap-1 mt-1 px-1.5">
                <span className="text-[6.5px] font-mono font-bold uppercase tracking-[0.15em] flex items-center gap-1.5">
                  ...
                  {msg.telemetry?.latency && (
                    <>
                      <span className="text-zinc-800">•</span>
                      <span className="text-[5.5px] text-indigo-400 opacity-60">{msg.telemetry.latency}ms</span>
                    </>
                  )}
                  <span className="text-zinc-800">•</span>
          ...
          <div className="absolute inset-0 z-[60] bg-black/98 backdrop-blur-3xl flex flex-col items-center justify-center p-6 safe-area-pb animate-in fade-in">
            ...
            <div className="mt-5 text-center space-y-2">
                ...
                <div className="h-4 overflow-hidden">
                  <p className={`text-[7px] uppercase tracking-[0.4em] animate-pulse ${sessionStatus === 'recovering' || sessionStatus === 'error' ? 'text-red-400' : 'text-indigo-400'}`}>
        ...
        {showSettings && (
          <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-3xl flex items-end justify-center animate-in fade-in" onClick={() => setShowSettings(false)}>
            <div className="glass-premium pearlescent w-full max-w-lg rounded-t-[1.8rem] p-5 pt-8 space-y-5 animate-in slide-in-from-bottom-20 overflow-y-auto max-h-[92vh] border-t border-white/20 relative overscroll-contain" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-start sticky top-[-2px] bg-transparent z-[110] pb-4 backdrop-blur-sm">
                ...
                <button onClick={() => setShowSettings(false)} className="p-2 rounded-full glass-dark text-zinc-500 hover:text-white transition-colors">&times;</button>
              </div>
